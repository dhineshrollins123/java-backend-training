...
import com.service.pqrmicroservice.domain.Pqr;
...
public class PqrController {
    private final List<Pqr> pqrs = List.of(
            ...
            new Pqr(3L, "Sangi", LocalDate.of(2020, 8, 10)),
            new Pqr(4L, "dhinesh", LocalDate.of(2020, 6, 3)),
...
@PostMapping
    public ResponseEntity<Pqr> post(@RequestBody Pqr pqr){
        var pqrTwoDaysLater=new Pqr(
                pqr.getId(),
                pqr.getName(),
                pqr.getDate().plus(2, ChronoUnit.DAYS)
        );
        return  ResponseEntity.ok(pqrTwoDaysLater);
}
    @GetMapping("/{st}/{ed}")
